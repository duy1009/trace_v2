...
class resnet18(torch.nn.Module):
    def __init__(self, pretrained=False, freeze=False, se_module=False):
        ...
        if se_module:
            resnet_pretrained_features = pretrainedmodels.se_resnet18(num_classes=1000, pretrained="imagenet")
        else:
            resnet_pretrained_features = models.resnet18(pretrained=pretrained)
...
class resnet18d(torch.nn.Module):
    def __init__(self, pretrained=False, freeze=False, se_module=False):
        ...
        if pretrained:
            self.load_state_dict(model_zoo.load_url(model_urls["resnet18"]), strict=False)
        else:
            init_weights(self.slice1.modules())
            init_weights(self.slice2.modules())
            init_weights(self.slice3.modules())
...
if __name__ == "__main__":
    model = resnet18(False, False, False)
