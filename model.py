...
class TraceModel_v2(nn.Module):
    def __init__(
        ...
        super(TraceModel_v2, self).__init__()

        """ Base network """
        if dilated:
            self.basenet = resnet34d(pretrained, freeze, se_module)
        else:
            self.basenet = resnet34(pretrained, freeze, se_module)
        """ U network """
